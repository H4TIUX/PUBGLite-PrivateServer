...
#include "../NameCollisions.inl"
	...
	*/
	namespace Offsets
	{
		constexpr int32 GObjects = 0x0443E848;
		constexpr int32 AppendString = 0x015E0E60;
		constexpr int32 GNames = 0x00000000;
		constexpr int32 GWorld = 0x04694528;
		constexpr int32 ProcessEvent = 0x0176CC70;
		constexpr int32 ProcessEventIdx = 0x0000003F;
	}
...
{ \
	...
	{
	public:
		static constexpr auto DecryptPtr = [](void* ObjPtr) -> uint8_t* { uint64 v7 = reinterpret_cast<uint64>(ObjPtr); uint64 v17; uint32_t low = static_cast<uint32_t>(v7 - 829357123); low = _rotl(low, 16) - 1835374268; low = _rotl(low, 16) ^ 0x29096587; uint32_t high = static_cast<uint32_t>(v7 >> 32); high = _rotr(high + 868271848, 8) + 1994878927; high = _rotr(high, 8) ^ 0x57D95719; v17 = (static_cast<uint64>(high) << 32) | low; return reinterpret_cast<uint8_t*>(v17); };
		...
		int32 : 32;  // i put this padding manually here cuz ObjectNum() is at +0x10, not +0xC
	...
	public:
		...
		inline UObject* GetByIndex(int32 Index) const
		{
			uintptr_t addr = InSDKUtils::GetImageBase() + 0x443E848;

			uintptr_t encrypted =
				*reinterpret_cast<uintptr_t*>(addr);

			uint8_t* BasePtr =
				reinterpret_cast<uint8_t*>(Dec::objobjects(encrypted));
	...
	public:
		...
		int32 GetCompIdx() const {
			return Dec::comparison_id(ComparisonIndex);
		}
