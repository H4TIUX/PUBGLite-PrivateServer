...
{
public:
	static struct FTransform K2_LookAt(const struct FTransform& CurrentTransform, const struct FVector& TargetPosition, const struct FVector& LookAtVector, uint8 bUseUpVector, const struct FVector& UpVector, float ClampConeInDegree);
	static void K2_TwoBoneIK(const struct FVector& RootPos, const struct FVector& JointPos, const struct FVector& EndPos, const struct FVector& JointTarget, const struct FVector& Effector, struct FVector* OutJointPos, struct FVector* OutEndPos, uint8 bAllowStretching, float StartStretchRatio, float MaxStretchScale);
...
public:
	...
	void OnMontageBlendingOut(class UAnimMontage* Montage, uint8 bInterrupted);
	void OnMontageEnded(class UAnimMontage* Montage, uint8 bInterrupted);
